...
void SetDisplayMode(int modeToDisplay) {
		...
		case GAME_SCREEN: glClearColor(0, 0, 0, 1);break;  //game screen black
		case MENU_SCREEN : glClearColor(1, 0 , 0, 1);break; //menu screen red
	}
}
...
void DrawAlienEyes()
{
	...
	glScalef(2.5,4,0);
	glutSolidSphere(1,20,30);   //void glutWireSphere(GLdouble radius,GLint slices, GLint stacks);
	glPopMatrix();
...
void DrawSteeringWheel()
{
	...
	glutWireSphere(1,8,8);   // glutWireSphere render a wireframe sphere.
	glPopMatrix();
...
void DrawLazerBeam() {
	...
	float mouseXEnd = -((- mouseX) + xOne); //mousex,y cursor co-ordinates & x,yOne spaceship co-ordinates.
...
void DrawStone(int StoneIndex)
{
	...
	case 0:
		...
		glTranslated(xStone[index] , yStone[index] ,0);   //xstone,ystone are co-ordinates of stones.
